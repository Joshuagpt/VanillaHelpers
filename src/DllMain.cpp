...
static void __fastcall CGGameUI_Shutdown_h() {
    // Enter soft-fail mode BEFORE the client tears down WMO groups / SGroupPtr trees.
    // After Orgrimmar->Kargath (and similar routes) residual portal/group blocks may
    // already be inconsistent; free paths must not raise ERROR #124 on exit.
    Allocator::SetShuttingDown(true);
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID lpReserved) {
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        // lpReserved != nullptr -> process is terminating (ExitProcess). Other threads may
        // already be dead; unhooking while residual frees still run is dangerous and
        // unnecessary (address space is about to disappear).
        // lpReserved == nullptr -> explicit FreeLibrary; safe to clean hooks.
